...
//update produtos set valor_unitario = valor_unitario + ((valor_unitario * 5) / 100)
class OrcamentosController extends AppController
{
    ...
    public function index()
    {
        if ($this->user[0]['acesso'] == 2) {
            $res = $this->connection->newQuery()
                ->select('orcamentos.id, ps.nome as revendedor, orcamentos.dt_cadastro, status, motivo_cancelamento, valor_total, pessoas.nome, pessoas.sobrenome, pessoas.tipo, usuarios.acesso as acesso')
                ->from('orcamentos')
                ->join(
                    [
                ...
                ->join(
                    [
                        'table' => 'pessoas ps',
                        'type' => 'LEFT',
                        'conditions' => 'ps.id = serralheiro_vendedores.serralheiro_id'
                    ]
                )
                ->join(
                    [
            ...
            $res = $this->connection->newQuery()
                ...
                ->where('1');
            if (!empty($serralheiro_id)) {
                $res = $res->andWhere('orcamento_pessoas.serralheiro_id = ' . $serralheiro_id);
            }
            if (!empty($month) || $month === '0') {
                $res = $res->andWhere('MONTH(orcamentos.dt_cadastro) = ' . $month);
            }
            if (!empty($year)) {
                $res = $res->andWhere('YEAR(orcamentos.dt_cadastro) = ' . $year);
            }
    ...
    public function novo(){
        ...
        $appVer = $this->request->getData('appVer');

        if (!empty($serralheiro_id) && !empty($cliente_id) && !empty($orc)) {
            //Prevenindo que versões anteriores possão gerar orçamentos com mais de uma porta
            if(isset($orc['portas'])){
                if(empty($appVer)){
                    echo "ORÇAMENTO DESATIVADO PARA VERSÕES OBSOLETAS DO APLICATIVO, POR FAVOR ATUALIZE PARA A ÚLTIMA VERSÃO...";
                    exit;
                }
            }
            ...
            if (isset($orc['portas'])) {
                ...
                try{
                    ...
                    $this->excluirPorId($orc_id);
                    echo "Erro no orçamento/portão";
                    exit;
                }
            }
            ...
            try{
                ...
                $this->excluirPorId($orc_id);
                echo "Erro no orçamento/pessoas";
                exit;
            }
            ...
            if (!empty($orc['motor'])) {
                ...
                try{
                    $this->connection->insert('orcamento_produto', ['produto_id' => $res[0]['produto_id'], 'orcamento_id' => $orc_id, 'preco_unitario' => $valores['motor'], 'quantidade' => $quantidades['motor'], 'acrescimo' => $acrescimo]);
                }catch (\Exception $e) {
                    $this->excluirPorId($orc_id);
                    echo "Erro no orçamento/motor";
                    exit;
                }
            ...
            if (!empty($orc['testeira'])) {
                ...
                try{
                    $this->connection->insert('orcamento_produto', ['produto_id' => $res[0]['produto_id'], 'orcamento_id' => $orc_id, 'preco_unitario' => $valores['testeira'], 'quantidade' => $quantidades['testeira'], 'acrescimo' => $acrescimo]);
                }catch (\Exception $e) {
                    $this->excluirPorId($orc_id);
                    echo "Erro no orçamento/testeira";
                    exit;
                }
            }
            if (isset($orc['fita'])) {
                if (!empty($orc['fita'])) {
                    ...
                    try{
                        $this->connection->insert('orcamento_produto', ['produto_id' => $res[0]['produto_id'], 'orcamento_id' => $orc_id, 'preco_unitario' => $valores['fita'], 'quantidade' => $quantidades['fita'], 'acrescimo' => $acrescimo]);
                    }catch (\Exception $e) {
                        $this->excluirPorId($orc_id);
                        echo "Erro no orçamento/fita";
                        exit;
                    }
                }
            }
            ...
            if (!empty($orc['automatizador'])) {
                ...
                try{
                    $this->connection->insert('orcamento_produto', ['produto_id' => $res[0]['produto_id'], 'orcamento_id' => $orc_id, 'preco_unitario' => $valores['automatizador'], 'quantidade' => $quantidades['automatizador'], 'acrescimo' => $acrescimo]);
                }catch (\Exception $e) {
                    $this->excluirPorId($orc_id);
                    echo "Erro no orçamento/automatizador";
                    exit;
                }
            }
            if (isset($orc['entrada'])) {
                if (!empty($orc['entrada'])) {
                    ...
                    try{
                        $this->connection->insert('orcamento_produto', ['produto_id' => $res[0]['produto_id'], 'orcamento_id' => $orc_id, 'preco_unitario' => $valores['entrada'], 'quantidade' => $quantidades['entrada'], 'acrescimo' => $acrescimo]);
                    }catch (\Exception $e) {
                        $this->excluirPorId($orc_id);
                        echo "Erro no orçamento/entrada";
                        exit;
                    }
                }
            }
            if (isset($orc['pintura'])) {
                if (!empty($orc['pintura'])) {
                    ...
                    try{
                        ...
                        $this->excluirPorId($orc_id);
                        echo "Erro no orçamento/pintura";
                        exit;
                    }
                }
            }
                ...
                if (!empty($orc['opcionais'])) {
                    ...
                    foreach ($opcs as $opc) {
                        ...
                        try{
                            ...
                            $this->excluirPorId($orc_id);
                            echo "Erro no orçamento/opcionais";
                            exit;
                        }
                    }
                }
    ...
    public function abrir()
    {
        ...
        if (count($orc) > 0) {
            ...
            $motivoCancelamento = $this->request->getData('motivo_cancelamento');
            if (!empty($altStatus) || $altStatus === "0") {
                if ($orc[0]['status'] != 2 || $this->user[0]['acesso'] == 1) {
                    if($altStatus == '0' && isset($motivoCancelamento)){
                        $this->connection->update('orcamentos', ['status' => $altStatus, 'motivo_cancelamento' => $motivoCancelamento], ["id" => $id]);
                    }else{
                        $this->connection->update('orcamentos', ['status' => $altStatus], ["id" => $id]);
                    }

                    $result = array('msg' => "Status alterado", 'AltStatus' => 1);

                    if(isset($motivoCancelamento)){
                        array_push($result, ['MotivoStatus' => 1]);
                    }
    ...
    {
        $orcamento_portao = $this->request->getData('orcamento_portao');
        $orcamento_portao = json_decode($orcamento_portao);

        $orcamento_produtos = $this->request->getData('orcamento_produtos');
        $orcamento_produtos = json_decode($orcamento_produtos);

        $valor_total = $this->request->getData('valor_total');

        $orc_id = $orcamento_portao->orcamento_id;

        $this->connection->update('orcamento_portao', [
            'portas'=>$orcamento_portao->portas,
            'altura' => $orcamento_portao->altura,
            'largura' => $orcamento_portao->largura,
            'tpi_id' => $orcamento_portao->tpi_id
        ], ['orcamento_id' => $orc_id]);

        $this->connection->delete('orcamento_produto', ['orcamento_id' => $orc_id]);

        foreach ($orcamento_produtos as $produto) {
            $this->connection->insert('orcamento_produto', [
        ...
        $this->connection->update('orcamentos', ['valor_total'=>$valor_total, 'pdf' => null], ['id' => $orc_id]);
    ...
    private function excluirPorId($id){
        try {
            $this->connection->delete('orcamentos', array('id' => $id));
        } catch (Exception $e) {
