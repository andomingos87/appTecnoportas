            ...
            <div class="panel-body">
                <form id='formPinturas' method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <div class="fileupload fileupload-new" data-provides="fileupload">
                            <div class="fileupload-new thumbnail" style="width: 200px; height: 150px;">
                                <label for='tbImg'><img src="<?php if (!empty($pintura['imagem'])) { echo $this->Layout->getLink('files?name='.$pintura['imagem']); } else { echo $this->Layout->getLink('files?name=sem-img.jpg'); } ?>" alt="Enviar uma imagem" /></label>
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <input id='tbCod' class="form-control" type='text' name='pintura[codigo]' value="<?= $pintura['codigo'] ?>"/>
                    </div>
                    <div class="form-group">
                        <label for='tbNome'>Nome:</label>
                        <input id='tbNome' class="form-control" type='text' name='pintura[nome]' value="<?= $pintura['nome'] ?>"/>
                    </div>
                    <div class="form-group">
                        <label for='taDesc'>Descrição:</label>
                        <textarea id='taDesc' rows="3"  class="form-control" name='pintura[descricao]'><?= $pintura['descricao'] ?></textarea>
                    </div>
                    <div class="form-group">
                        <label for='tbVu'>Valor por M2: (Metro Quadrado)</label>
                        <input id='tbVu' class="form-control" type='text' name='pintura[valor_unitario]' placeholder="0.00" value="<?= $pintura['valor_unitario'] ?>"/>
                    </div>
                    <div class="form-group">
                        ...
                        <label class="radio-inline">
                            ...
                            <input type="radio" value="<?= $fita['cor_id'] ?>" name="fitas" id="fita<?= $fita['cor_id'] ?>" <?= $fita['cor_id'] == $pintura['cor_id'] ? "checked='checked'" : "" ?>>
                        </label>
                        <?php endforeach; else: ?>
                    ...
                    <div class="form-group">
                        ...
                        <input id='tbGrupo' class="form-control" type='number' name='grupo' value='<?=$pintura["grupo_id"]?>'/>
                    </div>
