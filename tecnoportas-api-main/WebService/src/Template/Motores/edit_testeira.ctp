            ...
            <div class="panel-body">
                <form id='formTesteiras' method="POST" enctype="multipart/form-data" onsubmit="return checkTesteiras(event)">
                    <div class="form-group">
                        ...
                        <div class="fileupload fileupload-new" data-provides="fileupload">
                            <div class="fileupload-new thumbnail" style="width: 200px; height: 150px;">
                                <label for='tbImg'><img src="<?php if (!empty($testeira['imagem'])) { echo $this->Layout->getLink('files?name='.$testeira['imagem']); } else { echo $this->Layout->getLink('files?name=sem-img.jpg'); } ?>" alt="Enviar uma imagem" /></label>
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <input id='tbCod' class="form-control" type='text' name='testeira[codigo]' value="<?= $testeira['codigo'] ?>"/>
                    </div>
                    <div class="form-group">
                        <label for='tbNome'>Nome:</label>
                        <input id='tbNome' type='text' class="form-control" name='testeira[nome]' value="<?= $testeira['nome'] ?>"/>
                    </div>
                    <div class="form-group">
                        <label for='taDesc'>Descrição:</label>
                        <textarea id='taDesc' rows="3" class="form-control" name='testeira[descricao]'><?= $testeira['descricao'] ?></textarea>
                    </div>
                    <div class="form-group">
                        <label for='tbVu'>Valor:</label>
                        <input id='tbVu' class="form-control" type='text' name='testeira[valor_unitario]' placeholder="0.00" value="<?= $testeira['valor_unitario'] ?>"/>
                    </div>
                    <div class="form-group">
                        <div>
                        ...
                        <label class="checkbox-inline">
                            ...
                            <input type="checkbox" value="<?= $motores[$i]['id'] ?>" name="motores[]" id="motor<?= $motores[$i]['id'] ?>"<?= in_array($motores[$i]['id'], $tcats) ? " checked='checked'" : "" ?>>
                        </label>
                        <?php endfor; else: ?>
